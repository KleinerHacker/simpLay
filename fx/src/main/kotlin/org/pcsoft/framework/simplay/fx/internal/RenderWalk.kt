 ...
 * target surface. Used from IP-02 on to paint the sheet frame; unused here.
 */
internal fun interface PageFrameDecorator {
 ...
 * the document width. Used from IP-02 on to paint the dashed page-break line; unused here.
 */
internal fun interface PageSeparatorDecorator {
    fun decorate(gc: GraphicsContext, gapTop: Double, gapBottom: Double, width: Double)
}

/**
 * The measured-tree walk without a drawing surface: iterates the blocks, lines and parts of [page]
 * and reports each part's absolute position on the target surface (offset by [originX] / [originY]).
 *
 * [onBlockFont] fires once per block before its parts, [onPart] fires once per
 * [org.pcsoft.framework.simplay.engine.measure.MeasuredTextPart] with the text and its pen position
 * `(x, baselineY)` following `docs/docs/engine/rendering.md`. [renderPage] is the drawing consumer of
 * this walk.
 */
