...
import org.pcsoft.framework.playsim.engine.model.PageLayout
...
internal object MeasureTestData {
    ...
    val bodyMetrics = FontMetrics(ascent = 9.0, descent = 3.0, leading = 2.0)

    /** A left-aligned style over [bodyFont] with 1.5x line spacing and one extra unit of leading. */
    val bodyStyle = TextStyle(
        font = bodyFont,
        lineSpacing = LineSpacing(factor = 1.5, extraLeading = 1.0),
    ...
    fun measuredBodyFont() = MeasuredFont(bodyFont, bodyMetrics)

    /** Builds the [bodyStyle] wrapped as a [MeasuredTextStyle]. */
    fun measuredBodyStyle() = MeasuredTextStyle(bodyStyle, measuredBodyFont())

    /** A throwaway rectangle used where the concrete value does not matter. */
    fun someRect() = Rect(x = 0.0, y = 0.0, width = 10.0, height = 10.0)
     ...
     */
    fun measuredBlockReaching(bottom: Double) = MeasuredTextBlock(
        raw = TextBlock.of("Block", bodyStyle),
        lines = emptyList(),
        bounds = Rect(x = 0.0, y = 0.0, width = 100.0, height = bottom),
