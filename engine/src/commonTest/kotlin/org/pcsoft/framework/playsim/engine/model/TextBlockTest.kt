...
class TextBlockTest {
    ...
    /**
     * Verifies that [TextBlock.of] followed by [toString] reproduces a sentence whose words are
    ...
    /**
     * Verifies that [TextBlock.of] stores the tokenized parts in order and keeps the style.
     */
    @Test
    fun ofStoresTokenizedParts() {
        val block = TextBlock.of("one.", style)
        assertEquals(listOf(TextWord("one"), TextSymbol('.')), block.parts)
        assertEquals(style, block.style)
        assertEquals("one.", block.toString())
    }

    /**
     * Verifies that [TextBlock.of] carries the given style through to the block.
     */
    @Test
    fun ofCarriesStyle() {
