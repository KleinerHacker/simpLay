...
class CountingTest {
    ...
    private val layout = PageLayout(Size(100.0, 200.0), Margins(10.0, 10.0, 10.0, 10.0))
    ...
    fun countsOnPage() {
        val page = FlowPage(
            layout,
            listOf(TextBlock.of("one two", style), TextBlock.of("three.", style)),
        )
    ...
    fun countsOnDocument() {
        val document = Document(
            listOf(
                FlowPage(layout, listOf(TextBlock.of("a b", style))),
                SinglePage(layout, listOf(TextBlock.of("c d!", style))),
            ),
        )
