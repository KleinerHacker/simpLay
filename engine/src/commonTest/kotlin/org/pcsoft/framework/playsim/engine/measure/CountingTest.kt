...
class CountingTest {
    ...
    private fun block(text: String) = MeasuredTextBlock(
        raw = TextBlock.of(text, MeasureTestData.bodyStyle),
        lines = emptyList(),
    ...
    fun countsOnMeasuredDocument() {
        val document = MeasuredDocument(
            raw = org.pcsoft.framework.playsim.engine.model.Document(),
            pages = listOf(
                MeasuredFlowPage(FlowPage(layout = MeasureTestData.pageLayout), 0, listOf(block("a b"))),
                MeasuredSinglePage(SinglePage(layout = MeasureTestData.pageLayout), 1, listOf(block("c d!"))),
            ),
        )
