...
import org.pcsoft.framework.playsim.engine.model.SinglePage
...
class MeasuredAccessorTest {
    ...
    fun blockExposesMeasuredStyleOnly() {
        ...
        val block = MeasuredTextBlock(raw, emptyList(), MeasureTestData.someRect(), measuredStyle)
    ...
    fun pageExposesMeasuredBlocksAndContentArea() {
        val raw = FlowPage(layout = MeasureTestData.pageLayout)
        val block = MeasuredTextBlock(
            TextBlock.of("Page content", MeasureTestData.bodyStyle),
            emptyList(),
            MeasureTestData.someRect(),
            MeasureTestData.measuredBodyStyle(),
        )
        val page = MeasuredFlowPage(raw, pageIndex = 3, blocks = listOf(block))
    ...
    fun documentExposesMeasuredPagesOnly() {
        val rawFlow = FlowPage(layout = MeasureTestData.pageLayout)
        val rawSingle = SinglePage(layout = MeasureTestData.pageLayout)
        val raw = Document(pages = listOf(rawFlow, rawSingle))
        val measuredPages = listOf(
            MeasuredFlowPage(rawFlow, 0, emptyList()),
            MeasuredSinglePage(rawSingle, 1, emptyList()),
        )
        val document = MeasuredDocument(raw, measuredPages)
